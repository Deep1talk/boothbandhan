...
import { exportDirectLeadersForAdmin } from "@/lib/users/queries";
...
export async function GET(req) {
  try {
    ...
    const leaders = await exportDirectLeadersForAdmin(filters);
    ...
    return createCsvResponse("direct-leaders.csv", csv);
  } catch (error) {
    return errorResponse(500, error?.message || "Unable to export direct leaders");
  }
}
