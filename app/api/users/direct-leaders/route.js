...
import { getDirectLeadersForAdmin } from "@/lib/users/queries";
...
export async function GET(req) {
  try {
    ...
    if (user.role !== "Admin") {
      return errorResponse(403, "Only admins can view direct leaders");
    }

    const options = parseManagedUserListParams(req.nextUrl.searchParams);
    const data = await getDirectLeadersForAdmin(options);

    return successResponse(200, "Direct leaders fetched successfully", {
      ...data,
    });
  } catch (error) {
