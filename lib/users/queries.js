...
function buildManagedUserMatch(filters = {}) {
  const conditions = [];
  ...
  if (searchTerm) {
    ...
    conditions.push({
      $or: [{ name: regex }, { phone: regex }, { idNo: regex }, { block: regex }],
    });
  }
...
async function findUsersForExport(baseQuery, filters = {}) {
  const query = {
    ...baseQuery,
    ...buildManagedUserMatch(filters),
  };
...
async function findUsersPage(baseQuery, options = {}) {
  const page = options.page || 1;
  const pageSize = options.pageSize || 10;
  const query = {
    ...baseQuery,
    ...buildManagedUserMatch(options.filters),
  };
