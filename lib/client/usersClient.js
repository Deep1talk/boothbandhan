...
export async function resendManagedUserVerificationEmail(userId) {
  const { data: response } = await axios.post(`/api/users/${userId}/resend-verification`);
  return response;
}

export async function deleteManagedUser(userId) {
  const { data: response } = await axios.delete(`/api/users/${userId}`);
  return response;
}
...
export async function getAdminAllLeaderProblems(options = {}) {
  const { data: response } = await axios.get("/api/help-desk/leaders/problems", {
    params: Object.fromEntries(buildManagedUserQueryParams(options)),
  });
  return response.data;
}
